# ●	V originálnom zadaní zmeňte výber servera tak, aby sa vyberal ten s najmenčím poradovým číslom (poradové čísla sa určia hneď na začiatku)
...
def assign_patients(current_time, servers, queue, event_list, waiting_times, queue_length_history):
    """
    Priraďovanie čakajúcich pacientov k voľným serverom podľa poradia ich identifikačných čísel (od najmenšieho)
    """
    # Kým je vo fronte aspoň jeden pacient a aspoň jeden server je voľný, priraďujeme
    assigned = True
    while assigned and len(queue) > 0:
        assigned = False  # resetujeme príznak pre každý cyklus
        # Prechádzame servery zoradené podľa id (od najmenšieho)
        for server in sorted(servers, key=lambda s: s.id):
            # Ak server je voľný a vo fronte sú čakajúci pacienti, priraď ho
            if not server.is_busy and len(queue) > 0:
                ...
                schedule_event(event_list, p.departure_time, 'departure', server.id)
                server.is_busy = True
                server.next_free_time = p.departure_time
                server.busy_time += service_time
                server.count_served += 1
                assigned = True
    # Ak žiadny server nie je voľný, končíme priraďovanie.
...
def simulation():
    ...
    while event_list:
        ...
        if event_type == 'arrival':
            ...
            assign_patients(current_time, servers, queue, event_list, waiting_times, queue_length_history)
            ...
            # Po uvoľnení servera priradíme ďalšieho pacienta, ak niekto čaká
            assign_patients(current_time, servers, queue, event_list, waiting_times, queue_length_history)
